...
<div style="display:grid;grid-template-columns:300px 1fr;gap:12px;align-items:start">
    ...
    <div style="background:var(--surface);border:.5px solid var(--border);border-radius:8px;padding:14px">
        <div style="font-size:12px;font-weight:500;color:var(--text-2);margin-bottom:10px">Request leave</div>
        ...
        <div x-show="warning" x-cloak x-text="warning"
             style="margin-bottom:9px;background:var(--warning-soft);border:.5px solid var(--warning-border);border-radius:6px;padding:7px 9px;font-size:11px;color:var(--warning-2)"></div>
        <button type="submit" style="width:100%;height:34px;background:var(--success-soft);color:var(--success);border:.5px solid var(--success-border);border-radius:6px;font-size:12px;font-weight:500;cursor:pointer">Submit request</button>
        </form>
    </div>
    @endcan
    ...
    <div>
        <div style="background:var(--surface);border:.5px solid var(--border);border-radius:8px;padding:14px;margin-bottom:12px">
            <div style="font-size:12px;font-weight:500;color:var(--text-2);margin-bottom:9px">Balance {{ $year }}</div>
            <div style="display:flex;gap:18px;flex-wrap:wrap">
                @foreach($balances as $b)
                <div>
                    <div style="font-size:10px;color:var(--text-3)">{{ $b['label'] }}</div>
                    @if($b['tracked'])
                    <div style="font-size:15px;font-weight:500;color:{{ $b['remaining'] <= 0 ? 'var(--danger)' : 'var(--success)' }}">
                        {{ rtrim(rtrim(number_format($b['remaining'],1),'0'),'.') }}<span style="font-size:11px;color:var(--text-4)"> / {{ rtrim(rtrim(number_format($b['entitled'],1),'0'),'.') }}</span>
                    </div>
                    @else
                    <div style="font-size:12px;color:var(--text-3);padding-top:3px">unpaid</div>
                    @endif
                </div>
                @endforeach
            </div>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px">
            ...
            <tr style="border-bottom:.5px solid var(--surface-3)">
                <td style="padding:8px 12px"><span style="font-size:10px;padding:2px 7px;border-radius:10px;background:var(--primary-soft);color:var(--primary-text)">{{ ucfirst($l->type) }}</span></td>
                <td style="padding:8px 12px;color:var(--text-3)">{{ $l->from_date?->format('d M') }}</td>
                <td style="padding:8px 12px;color:var(--text-3)">{{ $l->to_date?->format('d M Y') }}</td>
                <td style="padding:8px 12px;color:var(--text)">{{ $l->days }}</td>
                <td style="padding:8px 12px"><span style="font-size:10px;padding:2px 7px;border-radius:10px;background:{{ ['pending'=>'var(--warning-soft)','approved'=>'var(--success-soft)','rejected'=>'var(--danger-soft)'][$l->status]??'var(--surface-2)' }};color:{{ ['pending'=>'var(--warning)','approved'=>'var(--success)','rejected'=>'var(--danger-text)'][$l->status]??'var(--text-2)' }}">{{ ucfirst($l->status) }}</span></td>
                <td style="padding:8px 12px">
                    @can('hrm.self.leave')
                    @if($l->status === 'pending')
                    <form method="POST" action="{{ route('my.leave.destroy', $l->id) }}" onsubmit="return confirm('Withdraw this request?')">
                        @csrf @method('DELETE')
                        <button type="submit" title="Withdraw" style="width:24px;height:24px;background:var(--surface-2);border:.5px solid var(--border);border-radius:5px;color:var(--danger);cursor:pointer"><i class="ti ti-x" style="font-size:12px"></i></button>
                    </form>
                    @endif
                    @endcan
                </td>
            ...
            @empty
            <tr><td colspan="6" style="padding:26px;text-align:center;color:var(--text-4)">No leave requests yet.</td></tr>
            @endforelse
            </tbody>
        </table>
        </div>
