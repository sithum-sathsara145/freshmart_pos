...
<table style="width:100%;border-collapse:collapse;font-size:12px">
    ...
    <tr style="border-bottom:.5px solid #1a1d2a">
        <td style="padding:8px 12px;color:#4ade80">{{ $s->invoice_no }}</td>
        <td style="padding:8px 12px;color:#64748b">{{ $s->created_at->format('d M Y') }}</td>
        <td style="padding:8px 12px;color:#e2e8f0">{{ $s->customer?->name??'Walk-in' }}</td>
        <td style="padding:8px 12px;color:#94a3b8">{{ $s->user?->name }}</td>
        <td style="padding:8px 12px;color:#e2e8f0;font-weight:500">Rs. {{ number_format($s->total) }}</td>
        <td style="padding:8px 12px"><span style="font-size:10px;padding:2px 7px;border-radius:10px;background:{{ $s->status==='paid'?'#14532d':'#451a03' }};color:{{ $s->status==='paid'?'#4ade80':'#fb923c' }}">{{ ucfirst($s->status) }}</span></td>
    </tr>
    @empty
    <tr><td colspan="6" style="padding:32px;text-align:center;color:#4a5568">No sales in this period</td></tr>
    @endforelse
    </tbody>
</table>
</div>
